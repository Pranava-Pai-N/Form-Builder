...
function PublicSurveyPage() {
    ...
    if (notFound || !survey) {
        return (
            <div className="rounded-3xl border border-slate-800 bg-slate-900/90 p-10 text-center text-slate-300">
                ...
                <Link to="/" className="mt-6 inline-flex rounded-full bg-indigo-500 px-5 py-3 text-sm font-semibold text-white hover:bg-indigo-400">
                    Back to home
                </Link>
            </div>
        )
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        submitSurveyResponse(
            survey.id,
            guestId,
            survey.questions.map((question) => ({ questionId: question.id, value: answers[question.id] ?? '' }))
        )
        setSubmitted(true)
    }
    ...
    return (
        <div className="mx-auto max-w-3xl space-y-8 rounded-3xl border border-slate-800 bg-slate-900/95 p-8 shadow-2xl shadow-slate-950/20">
            <div className="rounded-3xl overflow-hidden bg-slate-800">
                <div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url(${survey.coverImage})` }} />
                <div className="space-y-3 p-8" style={{ borderTop: `1px solid rgba(148, 163, 184, 0.1)` }}>
                    <div className="flex items-center gap-3">
                        <span className="h-4 w-4 rounded-full" style={{ backgroundColor: survey.primaryColor }} />
                    ...
                    <p className="text-slate-400">{survey.description ?? 'Share this form with anyone to start collecting feedback.'}</p>
                </div>
            </div>
                ...
                {survey.questions.map((question) => (
                    <div key={question.id} className="rounded-3xl border border-slate-800 bg-slate-950/90 p-6">
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                <p className="text-sm text-slate-400">{question.type === 'short_text' ? 'Short answer' : question.type === 'multiple_choice' ? 'Multiple choice' : 'Rating'}</p>
                                <p className="mt-2 text-lg font-semibold text-white">{question.text}</p>
                            </div>
                            {question.isRequired ? <span className="rounded-full bg-rose-500/10 px-3 py-1 text-xs text-rose-300">Required</span> : null}
                        </div>
            ...
            <p className="text-center text-sm text-slate-500">Your responses are saved locally in this browser.</p>
        </div>
    )
}
